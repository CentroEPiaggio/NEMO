%%%Count for non-fluorescent cells%%%
function [number] = count_noflu(image)    
...
%Count
%im = imerode(im_bw, strel('diamond', 6));
%im = bwareaopen(im, 18, 8);   %removes object that have fewer than 8 pixels
%im = bwmorph(im, 'spur');    %removes spur pixels
im =~im_bw;    %per GR eliminando le 3 righe precedenti
[im,num] = bwlabel(im, 8);   
...
figure, imshow(im)    %%così posso vedere se effettivamente in bianco sono
                                  %%evidenziate le cellule

answer = strcat('Cells number:  ', num2str(number));
