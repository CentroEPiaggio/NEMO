%
% [xFit yFit yError y0 A lambda x0 sFormula] = myFitLogRS(xData, yData, resamplingCount, sFormulaMode);
%
% [ABSTRACT]
% data FITting using LOGarithmic Raising Saturated model: y0 + A * log( 1 + lambda*(x-x0))
%
% [INPUT]
...
%
function [xFit yFit yError y0 A lambda x0 sFormula] = myFitLogRS(xData, yData, resamplingCount, sFormulaMode)
...
yError = sum(((yDataFit - yData)./yData).^2)/length(xData);
	...
	function [yValues] = myFormula(xValues, params)
		...
		%
		yValues = params(1) + params(2) * log(1+params(3)*(xValues-params(4)));
	end
	...
	function [sFormula] = myFormulaString(sFormulaMode, params)
		...
		switch(sFormulaMode)
			case 'tex'
				sFormula = sprintf('%.04f + %.04f \\cdot \\{ log[1 + (x - %.04f) / %.04f] \\}', params(1), params(2), params(4), 1/params(3));
			case 'latex'
				sFormula = sprintf('$%.04f + %.04f \\cdot [ log(1 + \\frac{x-%.04f}{%.04f}) ]$', params(1), params(2), params(4), 1/params(3));
			case 'none'
				sFormula = sprintf('%.04f + %.04f * \\{log[1 + (x - %.04f) / %.04f]\\}', params(1), params(2), params(4), 1/params(3));
		end
	end
